import pandas as pd
from src.utils.data_utils import format_currency, format_date
...
def tratar_valores_df(df: pd.DataFrame, currency_cols=None, date_cols=None, fill_map=None):
    ...
    for col in df.columns:
        if col in currency_cols or any(v in col.lower() for v in ["valor", "inadimplência"]):
            df[col] = df[col].apply(lambda x: "R$ 0,00" if pd.isna(x) or x in [None, 0, 0.0, "0", "0.0", "nan", "None"] else format_currency(x))
        if col in date_cols or "data" in col.lower():
            df[col] = df[col].apply(lambda x: "Data não informada" if pd.isna(x) else format_date(x))
        if col in fill_map:
