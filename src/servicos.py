...
def criar_rascunho_graph(token_acesso: str, destinatario: str, assunto: str, corpo: str, anexos: List[caminho]) -> bool:
    ...
    for caminho_anexo in anexos:
        if caminho_anexo and caminho_anexo.exists():
            try:
                tamanho_arquivo = caminho_anexo.stat().st_size
                if tamanho_total_anexos + tamanho_arquivo > LIMITE_TAMANHO_ANEXO_MB * 1024 * 1024:
                    logging.warning(f"Anexo {caminho_anexo.name} excede o limite de tamanho total e não será adicionado.")
                    st.warning(f"Anexo {caminho_anexo.name} excede o limite de tamanho e não foi adicionado.")
                    continue
                conteudo_bytes = caminho_anexo.read_bytes()
                ...
                logging.error(f"Erro ao processar anexo {caminho_anexo.name}: {e}", exc_info=True)
                st.warning(f"Não foi possível processar o anexo {caminho_anexo.name}: {e}")
        else:
            logging.warning(f"Anexo não encontrado ou inválido ao preparar API: {caminho_anexo}")
    try:
